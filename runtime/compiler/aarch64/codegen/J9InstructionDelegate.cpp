...
static bool
doesInstructionThrowNPE(TR::InstOpCode::Mnemonic opCodeValue)
{
   return (opCodeValue != TR::InstOpCode::prfm) && (opCodeValue != TR::InstOpCode::prfmimm) && (opCodeValue != TR::InstOpCode::prfmoff);
}
...
setupImplicitNullPointerExceptionImpl(TR::CodeGenerator *cg, TR::Instruction *instr, TR::Node *node, TR::MemoryReference *mr)
   ...
   if(cg->getHasResumableTrapHandler())
      ...
      // 3. The instruction throws NPE (prefetch does not throw it).
      ...
      if(node != NULL & mr != NULL &&
         ...
         doesInstructionThrowNPE(instr->getOpCodeValue()) &&
