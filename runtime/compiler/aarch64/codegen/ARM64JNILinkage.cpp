...
void J9::ARM64::JNILinkage::acquireVMAccess(TR::Node *callNode, TR::Register *vmThreadReg, TR::Register *scratchReg0, TR::Register *scratchReg1, TR::Register *scratchReg2)
   ...
   //
   generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addimmx, callNode, scratchReg0, vmThreadReg, fej9->thisThreadGetPublicFlagsOffset());
   generateTrg1ImmInstruction(cg(), TR::InstOpCode::movzx, callNode, scratchReg1, fej9->constAcquireVMAccessOutOfLineMask());
...
void J9::ARM64::JNILinkage::releaseVMAccessAtomicFree(TR::Node *callNode)
   {
   TR_UNIMPLEMENTED();
   }

void J9::ARM64::JNILinkage::acquireVMAccessAtomicFree(TR::Node *callNode)
   {
   TR_UNIMPLEMENTED();
   }
...
void J9::ARM64::JNILinkage::buildJNICallOutFrame(TR::Node *callNode, bool isWrapperForJNI, TR::LabelSymbol *returnAddrLabel,
   ...
   if (fej9->needClassAndMethodPointerRelocations())
      ...
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::movkx, callNode, scratchReg0, ((methodAddr >> 32) & 0x0000ffff) | (TR::MOV_LSL16 * 2) , cursor);
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::movkx, callNode, scratchReg0, (methodAddr >> 48) | (TR::MOV_LSL16 * 3) , cursor);
...
void J9::ARM64::JNILinkage::buildJNIArgs(TR::Node *callNode, TR::RegisterDependencyConditions *deps, bool passThread, bool passReceiver)
   {
   TR_UNIMPLEMENTED();
   }

TR::Register *J9::ARM64::JNILinkage::buildReturnDependencies(TR::Node *callNode, TR::RegisterDependencyConditions *deps)
   {
   TR_UNIMPLEMENTED();
   return NULL;
   }
   ...
   if (child->getOpCodeValue() == TR::loadaddr)
      ...
      if (sym)
         ...
         if (sym->isAddressOfClassObject())
            ...
            generateTestImmInstruction(cg(), child, whatReg, 0, true);
...
void J9::ARM64::JNILinkage::adjustReturnValue(TR::Node *callNode, bool wrapRefs, TR::Register *returnRegister)
   ...
   switch (resolvedMethod->returnType())
      ...
      case TR::Address:
         if (wrapRefs)
            {
            // unwrap the return object is non-null
...
void J9::ARM64::JNILinkage::checkForJNIExceptions(TR::Node *callNode, TR::Register *vmThreadReg, TR::Register *scratchReg)
   ...
   cg()->addSnippet(new (trHeapMemory()) TR::ARM64HelperCallSnippet(cg(), callNode, exceptionSnippetLabel, throwSymRef));
...
TR::Instruction *J9::ARM64::JNILinkage::generateMethodDispatch(TR::Node *callNode, bool isJNIGCPoint,
                                                               TR::RegisterDependencyConditions *deps, TR::Register *scratchReg, uintptrj_t targetAddress)
   {
   TR::ResolvedMethodSymbol *resolvedMethodSymbol = callNode->getSymbol()->castToResolvedMethodSymbol();
   TR_ResolvedMethod *resolvedMethod = resolvedMethodSymbol->getResolvedMethod();
   TR_J9VMBase *fej9 = reinterpret_cast<TR_J9VMBase *>(fe());
   ...
   if (fej9->needClassAndMethodPointerRelocations())
      ...
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::movkx, callNode, scratchReg, ((targetAddress >> 32) & 0x0000ffff) | (TR::MOV_LSL16 * 2) , cursor);
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::movkx, callNode, scratchReg, (targetAddress >> 48) | (TR::MOV_LSL16 * 3) , cursor);
      ...
      {
      loadConstant64(cg(), callNode, targetAddress, scratchReg);
      }
   // Add the first instruction of address materialization sequence to JNI call sites
   cg()->getJNICallSites().push_front(new (trHeapMemory()) TR_Pair<TR_ResolvedMethod, TR::Instruction>(resolvedMethod, cursor->getNext()));
...
TR::Register *J9::ARM64::JNILinkage::buildDirectDispatch(TR::Node *callNode)
   ...
   //TODO support GPU Helper
   bool isGPUHelper = false;
   ...
   buildJNIArgs(callNode, deps, passThread, passReceiver);

   TR::Register *returnRegister = buildReturnDependencies(callNode, deps);
   ...
   TR::Register *x11Reg = deps->searchPreConditionRegister(TR::RealRegister::x10);
   TR::Register *x12Reg = deps->searchPreConditionRegister(TR::RealRegister::x10);

   if (createJNIFrame)
      {
      //TODO support GPU Helper case
   ...
   TR::Instruction *callInstr = generateMethodDispatch(callNode, isJNIGCPoint, deps, x9Reg, targetAddress);
